...
int main(int argc, char **argv)
{
    // ros::init(argc, argv, "imu_receive");
     ros::init(argc, argv, "controller_calc");

    ros::NodeHandle n;
    ros::Subscriber sub = n.subscribe("/imu/data", 1000, chatterCallback_IMU);
    ros::Subscriber sub1 = n.subscribe("/vrpn_client_node/RigidBody007/pose", 1000, chatterCallback_Optitrack);
    ros::Subscriber sub2 = n.subscribe("/motor_signal", 1000, chatterCallback_MotorSignal);

    ros::Publisher pub = n.advertise<custom_pkg::control_signal>("/control_signal", 50);
	ros::Publisher pub1 = n.advertise<sensor_msgs::Imu>("/imu/data_filtered", 50);
    
    // Declaration for publishing the reference path
    // of "nav_msgs::path" type

    ros::Publisher pub_path = n.advertise<nav_msgs::Path>("/path_ref",50);
    ...
    // of "nav_msgs::path" type
    ros::Publisher pub_path_car = n.advertise<nav_msgs::Path>("/path_car",50);
    ...
    lane_center.LoadPathFromCSV("/home/suzlab/RC_ws_Car1/src/controller_calc/Path_Info/Test2.csv", true);
    lane_center = lane_center.ResampleConservative(0.01);
    lane_center.WritePath(ofstream("/home/suzlab/RC_ws_Car1/src/controller_calc/Path_Info/Test_resampled_path2.csv"));
    lane_center.OutputLaneForDebug("/home/suzlab/RC_ws_Car1/src/controller_calc/Path_Info/Test_detailed_path2.csv");
    ...
    double Tf = 0.15;	// Derivative filter's time constant
   ...
   std::cout << "debug3" << std::endl;
    ...
    string date_str(str);
    string filename = "/home/suzlab/data_exp/data_exp_" + date_str + ".csv";
...
//    myfile << "Time,px,py,pz,vx,vy,u,y_e,theta_e,theta_d_lane,ahead_theta_d,y_e_dot,theta_e_dot,delta_theta_d_lane,speed_pwm_cmd,steer_pwm_cmd,gyro_x,gyro_y,gyro_z,acc_x,acc_y,acc_z,angle_x,angle_y,angle_z,angle_Optitrack_x,angle_Optitrack_y,angle_Optitrack_z\n";
	...
	PID speed_PIDcontrol(0,0,0,30,-30);		// (Kp, Ki, Kd, error_integration_max, error_integration_min)
    ...
    while(ros::ok()) {
        ...
        if (Flag_control_start == 0) {
            ...
            speed_pwm_cmd = speed_PIDcontrol.calculate_Kp_variant(current_speed, speed_ref, x_PID_speed, 150, 4, 0.1, 1.5, dt, Flag_motor_start);
            //~ speed_pwm_cmd = speed_PIDcontrol.calculate(current_speed, speed_ref, x_PID_speed, dt, Flag_motor_start);
        }
        
        
			//~ steer_pwm_cmd = 800*y_e + 800*theta_e + 50*y_e_dot + 50*theta_e_dot;
            //speed_pwm_cmd = speed_PIDcontrol.calculate_Kp_variant(current_speed, speed_ref, x_PID_speed, 300, 10, 0.1, 1.5, dt, Flag_motor_start);
            //~ speed_pwm_cmd = speed_PIDcontrol.calculate(current_speed, speed_ref, x_PID_speed, dt, Flag_motor_start);
        ...
        //~ fprintf(stdout,"steer signal = %.2f \n", steer_pwm_cmd);
